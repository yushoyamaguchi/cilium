...
/* 
 * Compute checksum difference caused by rewriting fields in the inner IPv4 packet.
 *
 * This helper calculates the incremental checksum adjustment when the inner
 * IPv4 header (addresses, IP checksum) and L4 header (ports, L4 checksum) are
 * modified. The resulting diff can then be applied to the outer L4 checksum of
 * the encapsulating packet (e.g. ICMP error message) to keep it consistent.
 */
static __always_inline int
				  ...
				  struct ipv4_nat_entry **state)
{
	...
	*state = snat_v4_lookup(&tuple);
	...
	return snat_v4_rewrite_icmp_error_headers(ctx, tuple.nexthdr, (int)inner_l3_off, true, icmpoff,
